...
with app.app_context():
    ...
    # 2. 과목 데이터 설정
    courses_data = [
        # [필수/학부기초] 15학점 완벽 이수 (5과목 x 3학점)
        {"course_name": "컴퓨터 네트워크", "category": "필수/학부기초", "credit": 3, "semester": 1},
        ...
        # [전공선택] 48학점 이수 (16과목 x 3학점 - 기준 45학점 초과 달성)
        ...
        # [교양] 32학점 이수 (10과목 x 3학점 + 1과목 x 2학점)
    ...
    if not student:
        student = Student(student_id=STUDENT_ID, name="이영희", major="컴퓨터공학과", grade=4)
        db.session.add(student)
        db.session.commit()

    # 4. 수강 내역 초기화 후 전체 과목 이수 등록
    ...
    for c in courses_data:
        ...
        if course:
            ...
            elif course.category == "전공선택":
                ele_credits += course.credit
            elif course.category == "교양":
                gen_credits += course.credit

    db.session.commit()
    print("✅ Student2 시나리오 설정 완료! (졸업 가능)")
    print(f"   [총 취득 학점]: {total_credits} / 130")
    print(f"   [필수/학부기초]: {req_credits} / 15  (모두 이수)")
    print(f"   [전공 선택]: {ele_credits} / 45")
    print(f"   [교양 영역]: {gen_credits} / 32")